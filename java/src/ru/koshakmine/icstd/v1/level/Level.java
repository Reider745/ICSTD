...
import ru.koshakmine.icstd.v1.network.packets.PlaySoundPacket;
import ru.koshakmine.icstd.v1.network.packets.SpawnParticlePacket;
...
public class Level {
    ...
    public EntityItem spawnItem(float x, float y, float z, ItemStack stack) {
    ...
    public EntityItem spawnItem(Position position, ItemStack stack) {
        return spawnItem(position.x, position.y, position.z, stack);
    }
    ...
    public Entity[] listEntityInAABB(Position pos1, Position pos2, int type, boolean blackList){
        final long[] entities = region.fetchEntitiesInAABB(pos1.x, pos1.y, pos1.z, pos2.x, pos2.y, pos2.z, type, blackList);

        final Entity[] result = new Entity[entities.length];
        for (int i = 0;i < result.length;i++) {
            result[i] = Entity.from(entities[i]);
        }
    ...
    public Player[] getPlayersForRadius(Position pos, float radius){
        final long[] entities = region.fetchEntitiesInAABB(
                pos.x - radius, pos.y - radius, pos.z - radius,
                pos.x + radius, pos.y + radius, pos.z + radius,
                63, false);
        final Player[] players = new Player[entities.length];
    ...
    public void message(Position position, float radius, String message, String... formats){
    ...
    public void message(Position position, String message, String... formats) {
        message(position, VISUAL_RADIUS, message, formats);
    }
    ...
    public BlockEntity getBlockEntity(int x, int y, int z) {
        return (BlockEntity) BlockEntity.getManager().getBlockEntity(x, y, z, getDimensionId());
    }
